...
class Solution:
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        ...
        def land_flow(i, j, land):
            land.add((i, j))
            for x, y in ((i + 1, j), (i - 1, j), (i, j + 1), (i, j - 1)):
                if valid(x, y) and heights[x][y] >= heights[i][j] and (x, y) not in land:
                    land_flow(x, y, land)

        for i in range(len(heights)):
            land_flow(i, 0, p_land)
            land_flow(i, len(heights[0]) - 1, a_land)

        for i in range(len(heights[0])):
            land_flow(0, i, p_land)
            land_flow(len(heights) - 1, i, a_land)
    ...
    def pacificAtlantic_bfs(self, heights: List[List[int]]) -> List[List[int]]:
        ...
        for j in range(len(heights[-1])):
            a_ocean.add((len(heights) - 1, j))
            a_queue.append((len(heights) - 1, j))
        for i in range(len(heights)):
            a_ocean.add((i, len(heights[0]) - 1))
            a_queue.append((i, len(heights[0]) - 1))
        # print(p_ocean)  # {(0, 1), (4, 0), (0, 4), (0, 0), (0, 3), (2, 0), (3, 0), (0, 2), (1, 0)}
        # print(a_ocean)  # {(4, 4), (2, 4), (4, 0), (0, 4), (3, 4), (4, 3), (4, 2), (1, 4), (4, 1)}
        # p_ocean first
        while p_queue:
            row, col = p_queue.popleft()
            for r, c in ((row, col - 1), (row, col + 1), (row - 1, col), (row + 1, col)):
                if 0 <= r < len(heights) and 0 <= c < len(heights[0]):
                    if heights[r][c] >= heights[row][col] and (r, c) not in p_ocean:
                        p_ocean.add((r, c))
                        p_queue.append((r, c))
        while a_queue:
            ...
            for r, c in ((row, col - 1), (row, col + 1), (row - 1, col), (row + 1, col)):
                if 0 <= r < len(heights) and 0 <= c < len(heights[0]):
                    if heights[r][c] >= heights[row][col] and (r, c) not in a_ocean:
                        a_ocean.add((r, c))
                        a_queue.append((r, c))
        combine = p_ocean & a_ocean
        res = [[i, j] for i, j in combine]
        return res
